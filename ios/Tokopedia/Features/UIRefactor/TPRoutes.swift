    ...
    public static func configureRoutes() {
        ...
        JLRoutes.global().addRoute("/shop/:shopId") { (params: [String: Any]) -> Bool in
            ...
            isShopExists(nil, shopId: shopId, shopExists: { shopId in
                if shopId != nil {
                    navigator.navigateToShop(from: UIApplication.topViewController(), withShopID: shopId, withTrackerObject: tracker)
                } else {
        ...
        JLRoutes.global().addRoute("/hot/:hotName") { (params: [String: Any]) -> Bool in
            ...
            if let listName = filterParams["trackerListName"] as? String { trackerListName = listName }
            let tracker = ProductTracker(trackerAttribution: trackerAttribution, trackerListName: trackerListName)
            navigator.navigateToHotlistResult(from: UIApplication.topViewController(), withData: ["key": hotlistName], withTrackerObject: tracker)
            return true
        }
        ...
        JLRoutes.global().addRoute("/p/*") { (params: [String: Any]) -> Bool in
            ...
            if !pathComponent.isEmpty {
                ...
                var trackerAttribution: String = "none/other"
                if let attribution = filterParams["trackerAttribution"] as? String { trackerAttribution = attribution }
                navigator.navigateToIntermediaryCategory(from: UIApplication.topViewController(), withData: categoryDataForCategoryResultVC, withFilterParams: filterParams, trackerObject: ProductTracker(trackerAttribution: trackerAttribution))
            }
        ...
        JLRoutes.global().addRoute("/category/:categoryId") { (params: [String: Any]) -> Bool in
            ...
            var trackerAttribution: String = "none/other"
            if let attribution = filterParams["trackerAttribution"] as? String { trackerAttribution = attribution }
            navigator.navigateToIntermediaryCategory(from: UIApplication.topViewController(), withCategoryId: categoryId, categoryName: categoryName, isIntermediary: true, trackerObject: ProductTracker(trackerAttribution: trackerAttribution))
        ...
        JLRoutes.global().addRoute("/:shopName") { (params: [String: Any]) -> Bool in
            ...
            isShopExists(shopName, shopExists: { shopId in
                if shopId != nil {
                    navigator.navigateToShop(from: UIApplication.topViewController(), withShopName: shopName, withTrackerObject: tracker)
                } else {
        ...
        JLRoutes.global().addRoute("/product/:productId") { (params: [String: Any]) -> Bool in
            ...
            let productTracker = ProductTracker(trackerAttribution: filterParams["trackerAttribution"] as? String, trackerListName: filterParams["trackerListName"] as? String)
            
            NavigateViewController.navigateToProduct(from: UIApplication.topViewController(), withProductID: productId, andName: "", andPrice: "", andImageURL: "", andShopName: "", with: productTracker)
            return true
        }
        ...
        JLRoutes.global().addRoute("/:shopName/:productName") { (params: [String: Any]) -> Bool in
            ...
            isShopExists(shopName, shopExists: { shopId in
                if shopId != nil {
                    NavigateViewController.navigateToProduct(from: UIApplication.topViewController(), withProductID: "", andName: productName, andPrice: "", andImageURL: "", andShopName: shopName, with: tracker)
                } else {
                    openWebView(url as URL)
                }
            })
