...
class ChatListView extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.chatInbox.success) {
      if (!nextProps.chatInbox.data.paging_next) {
        this.setState({
          isRefreshing: false,
          isLoadingNextPage: false,
          currentPage: 1,
  ...
  handleScrolling = ({
    ...
    if (windowHeight + y >= height) {
      if (!this.state.isLoadingNextPage) {
        if (this.props.chatInbox.data.paging_next) {
          this.setState(
            {
            ...
            () => {
              this.props.fetchInboxList(this.state.currentPage)
            },
          )
        }
      }
  ...
  renderItemSearch = ({ item, section: { renderKey, resultFor }, index }) => (
    <SearchChatCell
      ...
      disabled={!this.props.connectedToInternet}
    />
  )
  ...
  renderFooter = () => {
    if (_.isEmpty(this.props.chatInbox.data.list) && this.props.fromIpad) {
      return null
    }

    if (_.isEmpty(this.props.chatInbox.data.list)) {
      return null
    }
  ...
  renderItem = ({ item, index }) => {
    ...
    return (
      <ChatCell
        ...
        currentMsgId={this.props.currentMsgId}
        unsetMsgId={this.props.unsetMsgId}
        editMode={this.props.chatInbox.isEditing}
        index={index}
        toggleSelectRow={this.props.toggleSelectRow}
        selectedData={this.props.chatInbox.selectedData}
        setIpadAttributes={this.props.setIpadAttributes}
        unsetIpadAttributes={this.props.unsetIpadAttributes}
        disabled={!this.props.connectedToInternet}
      />
  ...
  deleteSelectedChat = () => {
    AlertIOS.alert('Hapus chat?', 'Chat akan terhapus selamanya', [
      { text: 'Batal' },
      { text: 'Hapus', onPress: () => this.props.deleteSelectedData() },
    ])
  }

  selectedAllChat = () => {
    this.props.toggleSelectAllRow(_.isEmpty(this.props.chatInbox.selectedData))
  }
    ...
    return (
      <View style={styles.container}>
        <FlatList
          style={{
            flex: 1,
            marginBottom: this.props.chatInbox.isEditing ? 50 : 0,
          }}
          extraData={this.props.chatInbox.isEditing}
          data={this.props.chatInbox.data.list}
        ...
        {this.props.chatInbox.isEditing ? (
          <View
            style={{
              position: 'absolute',
              height: 50,
            ...
            <View style={{ flex: 1, flexDirection: 'row' }}>
              <View style={{ flex: 0.25 }}>
                ...
                >
                  <Text style={styles.textActive}>
                    {_.isEmpty(this.props.chatInbox.selectedData) ? (
                      'Pilih Semua'
                    ) : (
                      'Batal Pilih'
                    )}
              ...
              <View style={{ flex: 0.25 }}>
                <TouchableOpacity
                  ...
                  disabled={
                    _.keys(this.props.chatInbox.selectedData).length === 0
                  }
                >
                  <Text
                    style={
...
ChatListView.defaultProps = {
  currentMsgId: null,
}

ChatListView.propTypes = {
  currentMsgId: PropTypes.number,
  fromIpad: PropTypes.bool.isRequired,
  authInfo: PropTypes.object.isRequired,
  chatInbox: PropTypes.object.isRequired,
  chatSearch: PropTypes.object.isRequired,
  toggleSelectRow: PropTypes.func.isRequired,
  deleteSelectedData: PropTypes.func.isRequired,
  loadMoreSearchAllChat: PropTypes.func.isRequired,
  fetchReplyList: PropTypes.func.isRequired,
  fetchReplyListForSearch: PropTypes.func.isRequired,
  setIpadAttributes: PropTypes.func.isRequired,
  unsetIpadAttributes: PropTypes.func.isRequired,
  unsetMsgId: PropTypes.func.isRequired,
}
