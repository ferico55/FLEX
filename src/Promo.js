...
class Promo extends React.PureComponent {
  ...
  _loadingIndicator = () => {
    if (this.state.isLoading)
      return <ActivityIndicator animating={true} style={[styles.centering, { height: 44 }]} size="small" />
    else
      return null
  }
  ...
  _dropdownText = () => {
    switch (this.state.selectedCategory) {
      case 0: return 'Semua Promo'
      case 1: return 'Jual Beli Online'
      case 2: return 'Official Store'
      case 3: return 'Pulsa'
      case 4: return 'Tiket'
    }
  }
  ...
  _showDropdown = (e) => {
    ActionSheetIOS.showActionSheetWithOptions({
      options: ['Semua Promo', 'Jual Beli Online', 'Official Store', 'Pulsa', 'Tiket', 'Batal'],
      cancelButtonIndex: 5
    }, (index) => {
      if (index >= 5) return
      this.setState({
  ...
  _listHeader = () => {
    return <TouchableOpacity
      onPress={(e) => this._showDropdown(e)}>
      <View style={styles.dropDownWrapper} >
        <Text style={[styles.greyText, { fontWeight: '600' }]}>{this._dropdownText()}</Text>
        <Image source={{ uri: 'icon_arrow_down_grey' }} style={{ width: 14, height: 14, marginTop: 2, marginRight: 2 }} />
      </View>
    </TouchableOpacity>
  }
  ...
  _renderItem = (item, index) => {
    return <TouchableOpacity
      ...
      <View style={{ borderWidth: 0.5, borderColor: 'rgba(0,0,0,0.12)' }}>
        <PreAnimatedImage source={item.item.meta.thumbnail_image} style={styles.photo} onLoadEnd={() => { this.setNativeProps() }} />
        <View style={styles.textWrapper}>
          <View style={styles.promoWrapper}>
            <View style={{ flexDirection: 'row' }}>
              <Image source={{ uri: 'icon_stopwatch' }} style={styles.stopwatch} />
              <View style={{ flexDirection: 'column', marginRight: 15 }}>
                <Text style={[styles.greyText, styles.subtitle]}>Periode Promo</Text>
                <Text style={{ color: 'rgba(0,0,0,0.7)', fontSize: 14 }}>
                  {this._getPromoPeriod(item.item.meta.start_date, item.item.meta.end_date)}
                </Text>
              </View>
            </View>
            ...
            <View style={{ marginTop: 18, flexDirection: 'row' }}>
              ...
              <View style={{ flexDirection: 'column' }}>
                <View style={{ flexDirection: 'row' }}>
                  <Text style={item.item.meta.promo_code == "" ? [styles.greyText, styles.subtitle, { marginTop: 9 }] : [styles.greyText, styles.subtitle]}
                    numberOfLines={2}>
                    {item.item.meta.promo_code == "" ? "Tanpa Kode Promo" : "Kode Promo"}
                  </Text>
                  {item.item.meta.promo_code != "" &&
                    <TouchableOpacity
                      onPress={() => ReactInteractionHelper.showTooltip("Kode Promo",
                        ...
                        "icon_promo",
                        "Tutup")}>
                      <Image source={{ uri: 'icon_information' }}
                        resizeMode='contain'
                        style={item.item.meta.promo_code == "" ? [styles.info, { marginTop: 12 }] : styles.info} />
                    </TouchableOpacity>
                  }
                </View>
              ...
              <View style={{ flex: 1 }} />
              {item.item.meta.promo_code != "" &&
                <TouchableOpacity onPress={() => this._copyPromoCode(item.item.meta.promo_code)} >
  ...
  loadData(page = 1) {
    ...
    if (this.state.selectedCategory != 0) {
      ...
      featuredParams.categories = this._getCategoryID(this.state.selectedCategory)
    }
    ...
    if (page > 1) {
      promoRequest.subscribe((response) => {
        var dataSource = []
        if (response.code && response.code == 'rest_post_invalid_page_number' && response.data.status == 400) {
          this.setState({
            page: -1,
            isLoading: false
          });
      ...
      var request = Rx.Observable.zip(featuredPromoRequest, promoRequest)
