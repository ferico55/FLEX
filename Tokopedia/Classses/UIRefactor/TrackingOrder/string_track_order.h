//
//  string_track_order.h
//  Tokopedia
//
//  Created by Feizal Badri Asmoro on 2/8/15.
//  Copyright (c) 2015 TOKOPEDIA. All rights reserved.
//

#ifndef Tokopedia_string_track_order_h
#define Tokopedia_string_track_order_h

#define API_TRACKING_ORDER_PATH         @"tracking-order.pl"

#define API_ACTION_KEY                  @"action"
#define API_ACTION_TRACK_ORDER          @"track_order"
#define API_ORDER_ID_KEY                @"order_id"
#define API_USER_ID_KEY                 @"user_id"

#define API_STATUS_KEY                  @"status"
#define API_SERVER_PROCESS_TIME_KEY     @"server_process_time"
#define API_RESULT_KEY                  @"result"

#define API_TRACK_ORDER_KEY             @"track_order"

#define API_CHANGE_KEY                  @"change"
#define API_NO_HISTORY_KEY              @"no_history"
#define API_TRACK_HISTORY_KEY           @"track_history"
#define API_RECEIVER_NAME_KEY           @"receiver_name"
#define API_ORDER_STATUS_KEY            @"order_status"
#define API_DETAIL_KEY                  @"detail"
#define API_SHIPPING_REF_NUM_KEY        @"shipping_ref_num"
#define API_INVALID_KEY                 @"invalid"

#define API_DATE_KEY                    @"date"
#define API_STATUS_KEY                  @"status"
#define API_CITY_KEY                    @"city"

#define API_SHIPPER_CITY_KEY            @"shipper_city"
#define API_SHIPPER_NAME_KEY            @"shipper_name"
#define API_RECEIVER_CITY_KEY           @"receiver_city"
#define API_SEND_DATE_KEY               @"send_date"
#define API_RECEIVER_NAME_KEY           @"receiver_name"
#define API_SERVICE_CODE_KEY            @"service_code"

#endif
