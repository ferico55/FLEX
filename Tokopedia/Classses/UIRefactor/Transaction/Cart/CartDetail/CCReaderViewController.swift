...
class CCReaderViewController: UIViewController, CardIOViewDelegate{
    ...
    @IBAction func didSelectCancel(sender: UIButton) {
        AnalyticsManager.trackEventName("clickCardIOCancel", category: GA_EVENT_CATEGORY_CARDIO_SCAN, action: GA_EVENT_ACTION_CLICK, label: "Cancel Scan Credit Card")
    ...
    func cardIOView(cardIOView: CardIOView!, didScanCard cardInfo: CardIOCreditCardInfo!) {
        if ((cardInfo) != nil) {
            if let delegate = delegate {
                AnalyticsManager.trackEventName("clickCardIOSuccessScan", category: GA_EVENT_CATEGORY_CARDIO_SCAN, action: GA_EVENT_ACTION_CLICK, label: "Success Scan Credit Card")
