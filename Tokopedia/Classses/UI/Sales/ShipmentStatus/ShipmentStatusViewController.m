...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([_auth objectForKey:@"is_allow_manage_tx"] && order.order_detail.detail_ship_ref_num) {
...
{
    OrderTransaction *order = [_shipments objectAtIndex:indexPath.row];
    CGFloat rowHeight = tableView.rowHeight - 45;
    if ([_auth objectForKey:@"is_allow_manage_tx"] && order.order_detail.detail_ship_ref_num) {
        if (order.order_detail.detail_order_status == ORDER_DELIVERED ||
...
- (void)requestProcess:(id)object
{
    if (object && [object isKindOfClass:[RKMappingResult class]]) {
        ...
        NSDictionary *result = ((RKMappingResult*)object).dictionary;
        Order *orders = [result objectForKey:@""];
        [_shipments addObjectsFromArray:orders.result.list];

        _nextURI =  orders.result.paging.uri_next;
