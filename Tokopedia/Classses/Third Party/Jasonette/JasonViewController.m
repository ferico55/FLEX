                            ...
                            if([body[@"type"] isEqualToString:@"image"]){
                                ...
                                if(![url containsString:@"{{"] && ![url containsString:@"}}"]){
                                    ...
                                    [manager loadImageWithURL:[NSURL URLWithString:url]
                                                        ...
                                                        completed:^(UIImage *i, NSData* imageData, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                                                            download_image_counter--;
                                                            if(!error){
            ...
            if(chat_input[@"left"]){
                if(chat_input[@"left"][@"image"]){
                    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                        SDWebImageManager *manager = [SDWebImageManager sharedManager];
                        [manager loadImageWithURL:[NSURL URLWithString:chat_input[@"left"][@"image"]]
                                              options:0
                                             progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL* targetUrl) {
                                                 // progression tracking code
                                             }
